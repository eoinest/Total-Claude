...
export class Minimap {
  ...
  /**
   * A named route to yaw beside the two field routes (right drag, and Q/E): drag the rose to
   * turn, click it to face north. 0.014 rad per pixel puts a full revolution in 450 px of
   * travel, faster per pixel than a field drag because the rose is a stud rather than the
   * whole viewport.
   */
  ...
  draw(ctx: EngineContext): void {
    ...
    // The needle is a DOM element on the rose, because that control is also the grab handle
    // for turning, and because a transform written only when the integer bearing changes is
    // cheaper than redrawing two triangles and a disc every frame.
    //
    // The plate is drawn north-up and the view looks along +(sin yaw, cos yaw), so an arrow
    // drawn pointing up needs 180 - yaw of clockwise turn to point where the camera is
    // looking. The canvas needle this replaces rotated by -yaw, which is 180 degrees out:
    // it pointed at what was behind the camera.
