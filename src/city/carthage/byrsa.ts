...
import { byrsaGroundAt } from '../../maps/carthage/topography';
 ...
 *
 * ## The relief, and who owns it — **the terrain does, and this is now a `max`**
 *
 ...
 * the world footprint to 340 × 200 world m and takes the gradient from that — 1:3.8 on the
 * approach face.
 *
 * This file used to *synthesise* that hill, because there was no Carthage heightfield and a
 * camera at the citadel would otherwise have been forty-five metres underground. The
 * heightfield now carries it: `topography.ts` exports `byrsaGroundAt` and `BYRSA_FOOTPRINT`,
 * graded to the same 1:3.8 with `assertSurveyElevations` failing the build past 1:2.5. Two
 * Byrsas that disagree is exactly the class of bug this workstream keeps finding, so the
 * synthesis is gone and the seam is the `max` the map workstream specified:
 *
 *     byrsaReliefAt(heightAt, x, z)  ===  max(0, byrsaGroundAt(x, z) − heightAt(x, z))
 *
 * **Zero everywhere on Carthage**, because `regionalLand` already `max`es the hill into the
 * ground, and the full 45 m on any heightfield that does not — which is what keeps this
 * buildable on the Campus Martius under the development rig. Nothing downstream changed:
 * every consumer already read one function for its ground height.
 */
...
/**
 * Metres of hill the *city* must add on top of the terrain at a point.
 *
 * Zero on Carthage, where the heightfield carries the Byrsa; the full hill on a heightfield
 * that does not. `byrsaGroundAt` returns `-Infinity` outside the hill's skirt, so the `max`
 * degenerates to nothing there without a special case.
 */
export function byrsaReliefAt(
  heightAt: (x: number, z: number) => number, x: number, z: number
): number {
  return Math.max(0, byrsaGroundAt(x, z) - heightAt(x, z));
}

/**
 * The ground the citadel, its streets and its terraced housing all stand on.
 *
 * **One function, and everything on the hill reads it**, so a street, a terrace and a
 * revetment cannot end up at three different heights on the same slope. That was true when
 * the hill was synthesised here and it stays true now the terrain supplies it.
 */
export function carthageGroundAt(
  heightAt: (x: number, z: number) => number
): (x: number, z: number) => number {
  return (x, z) => Math.max(heightAt(x, z), byrsaGroundAt(x, z));
}
...
export function byrsaTopY(heightAt: (x: number, z: number) => number): number {
  return Math.max(heightAt(BYRSA.x, BYRSA.z), byrsaGroundAt(BYRSA.x, BYRSA.z));
}
...
/**
 * `groundAt` is the hill, not the plain: `carthageGroundAt(terrain.heightAt)`.
 *
 * The citadel is dimensioned off the ground it stands on and the ground now comes from the
 * heightfield, so a change to the hill in `topography.ts` moves the revetments, the temple
 * podium and the sixty steps with it rather than leaving them hanging.
 */
export function buildByrsa(groundAt: (x: number, z: number) => number): ByrsaOutput {
  const summitY = groundAt(BYRSA.x, BYRSA.z);
  // The hill at the citadel's toe, so the lowest revetment starts where the housing slope ends.
  const citadelToeY = groundAt(BYRSA.x + BYRSA.citadelHw, BYRSA.z);
