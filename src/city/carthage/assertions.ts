import type { CityAssertion, CityPlanRect } from '../cityPlan';
...
export function assertCarthage(inp: AssertInput): CityAssertion[] {
  ...
  //
  // Every solid against every solid: monument-monument, monument-house and house-house, with
  // nothing skipped for being `soft`. A broad-phase bin keeps it near-linear.
  {
    const CELL = 60;
    const bins = new Map<number, number[]>();
    const key = (x: number, z: number): number =>
      ((Math.floor(x / CELL) + 2048) << 12) | (Math.floor(z / CELL) + 2048);
    inp.footprints.forEach((f, i) => {
    ...
    for (const list of bins.values()) {
      for (let a = 0; a < list.length; a++) {
        for (let b = a + 1; b < list.length; b++) {
          ...
          seen.add(pair);
          const d = obbOverlap(inp.footprints[i], inp.footprints[j]);
          /**
          ...
          if (d > 0.4) {
            ...
            if (d > worst) {
              worst = d;
              const A = inp.footprints[i];
              const B = inp.footprints[j];
    ...
    out.push({
      ...
      ok: count === 0,
      detail: `${count} overlapping pair(s) over ${inp.footprints.length} solids, worst ${worst.toFixed(1)} m. `
        + `Population is every solid against every other — monuments, houses, quays and moles — `
        + `with a 0.4 m allowance for the party walls a terrace is made of, and no other exemption. `
