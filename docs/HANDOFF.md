...
## 21 Aug 2026 — the menus and loading screens got a picture

Branch `e/ui/cinematic-menu`. The owner: *"i think we should really try to improve the menu
screens and loading screens with dramatic images from battles and stuff. when you switch thru
different locations, it should like do some animation where you zoom over the maps in the
background."* All three parts are in.

**What is on screen now.** The front door and the setup screen sit over a full-bleed flyover of
whichever battlefield is selected — a nine-second seamless loop, ours, out of the real
simulation. Clicking a different battlefield crossfades and **pushes in from a 14% over-zoom**
over the new map while its clip restarts at the head of its own crane. The three battlefield
buttons carry 480x270 crops of the same plates, in colour when chosen and desaturated when not.
BEGIN BATTLE fades the menu out and the loading panel in **over the same photograph**: title
block lower left, progress bar under it, and a cited historical card opposite, rotating every
seven seconds. Nine plates, `public/menu/`, **2.99 MB**, regenerated by one command.

### The decision, and the measurement that made it

Pre-rendered, not live. `tools/scratch/menu-boot-cost.mjs` measured what a live camera over the
real terrain costs *before the player has committed to anything* — a cold page open, then BEGIN,
with the per-subsystem init read off the loading label:
...
The middle column is four to seven times the entire current cost of opening the page, on Apple
silicon over localhost with the texture set warm — **and it would be paid again on every click
of the map row**, which is the exact interaction that was asked to be a smooth flyover. It is
also not a flag: `main.ts` cannot construct `Engine` until the menu resolves, because the tier
fixes the soldier pool and the cascade count at `init`, so a live backdrop is a second engine and
a second WebGL context, thrown away at BEGIN.

**What would change that.** The tier constraint going away. If `Engine` could be built before the
menu, the honest design is a hybrid: the poster covers the first second and a live camera takes
over on the map being looked at. The number to re-take is the middle column.

### It did not make the page slower — nine interleaved cold loads per arm, 4G, production build

Interleaved A, B, A, B against one static server, because five other agents were on the machine
and one arm measured after the other gave 1,080-2,979 ms on the *same* build.

| | before | after |
|---|---:|---:|
| first contentful paint | 192 ms | 188 ms |
| menu interactive | 1,552 ms | **1,548 ms** |
| bytes by menu interactive | 755 kB | **760 kB** |
| bytes after eight seconds | 755 kB | 1,705 kB |
| long tasks >50 ms | 1.0/load, worst 693 ms | 1.0/load, worst 692 ms |

**The clip is never on the critical path** — at 4G the resource list at menu-interactive carries
no `/menu/*.webm` in any run. `dist` went 26.35 -> 29.22 MiB; the JS+CSS the browser must have
before anything happens went up **4.9 kB gzipped**. The dead `preconnect` to
`fonts.googleapis.com` in `index.html` is gone: nothing has ever fetched from that origin.

### `prefers-reduced-motion` is honoured twice, on purpose

`MenuBackdrop` refuses to fetch a clip at all, and `menu.css` kills the push-in and the ambient
drift in a media query — so a code path that forgets cannot bring the motion back. Measured: under
`reduce`, three AVIF posters and three thumbnails are fetched and **zero WebM**. A switch becomes
a 240 ms opacity crossfade between two stills, which is the right fallback: a hard cut between
two full-bleed photographs is a flash, which is worse for the same audience. Video is also
skipped on `saveData` and a 2G effective connection.

### What the studio said before anybody looked at a pixel

The first pass of `tools/shots/menu-plates.shot.mjs` was written by eye and two of its three
plates were unusable. `film.json` had already named the fault: **Rome at `sunAngle` 15.2** (inside
the 45-degree cone where every surface goes to one flat cream) and **Pydna at 168** (sun over the
camera's shoulder, every shadow hidden behind the thing casting it). Carthage at 69.5 was the only
one right. Swinging all three to 70-115 degrees is the entire difference between the two contact
sheets. *If a plate comes back washed out, read `sunAngle` before touching a light.*

### Feedback for the video studio

Four things wanted and not there. None blocked the pass; all four were worked around.

- **No VP9.** `film.mjs` encodes with Playwright's bundled ffmpeg, which is `libvpx`/VP8 only,
  and VP8 at a bitrate a menu can afford is mush. `tools/menu-plates.mjs` drives Chromium's
  `VideoEncoder` from a page instead, the way `trailer-encode.mjs` does — except that script
  hardcodes `const W = 1920, H = 1080`, so it could not be reused for a 1280x720 plate.
- **No `--frames`/still export at a chosen frame.** `--stills` gives three per shot at fixed
  positions; a poster has to be *the encoder's first output frame*, so the tool reads the JPEG
  sequence off disk itself.
- **`climbing` never fires on Carthage.** The predicate counts men on ladders and ramps, and the
  Punic assault storms up docked siege towers: it is flatly 0 for the whole battle while `melee`
  is in single figures. There is no finder for "the towers are docked and the columns are
  queuing", which is the Carthage picture, so that plate is on a clock and says so.
- **`ensureServer` reuses whatever is on its port.** Port 5921 held a `gpu-cost` worktree's vite
  from a previous day; the first probe run of this pass drove a *different branch's* menu for
  sixty seconds and timed out on a selector that does not exist there. It is the same class of
  failure `TC_VITE_CACHE_DIR` exists to prevent and the fix is the same shape: check the server
  is serving the tree you think it is, or take the port with `--strictPort` and fail loudly.

### Gate

tsc clean, lint 2/2, `qa-deploy` 33/33, `probe-seams` PASS both maps, and all three determinism
arms UNCHANGED at all seven checkpoints — 8,632 / 3,074 / 3,440. Nothing in `src/sim`, `src/ai`
or `src/units` was touched; the menu is outside the simulation and the plates are build output of
it. `src/ui/lore.ts` uses `Math.random` to pick which historical card comes up first, deliberately
and outside `check-determinism`'s scope: seeding it off the battle would make two `qa-determinism`
runs differ in their DOM for nothing.

### Left on the floor

- **The setup sheet is 1180 px wide in the middle of the screen**, so on a 1920 window the
  flyover is only visible in about 370 px down each side. The battlefield thumbnails carry the
  "make me want this one" job on that screen instead. A Rome II-style side-anchored panel would
  show the plate properly and is a redesign of a working screen, so it was not attempted.
- **The plates are 8.97 s each.** Long enough with a camera this slow; a 15 s loop would cost
  about 60% more bytes for a difference nobody has asked about.
- **No sound on the menu.** The mixer schedules against `AudioContext.currentTime` and there is
  no menu audio bus; a plate is silent.
- **`docs/ROME.md` §15 proposes retitling the Rome map** to "The Aurelian Wall, autumn 271",
  because §8.1 argues it is not a siege. If that lands, the front door subtitle and one loading
  card follow it automatically (both read `MapDefinition.subtitle`), but the `<title>` in
  `index.html` still says "Siege of Rome".
