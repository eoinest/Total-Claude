...
/**
 * **Converted in the assembly merge.** This branch was cut before the machine-wide browser cap
 * landed on `main`, so it opened a browser nobody could count and started no server of its own.
 * `npm run lint` (check 3 of 3) fails on that, and it is the right failure: twelve agents each
 * doing what this file does put the box at load average 160 on 16 cores. It now takes a slot and
 * brings its own vite, so the tree it grades is the tree it is standing in rather than whatever
 * happens to be answering on the port.
 */
const browser = await launchBrowser({
  label: 'probe-tiber', port: PORT, root: ROOT,
});
const { close: killServer } = await startVite({
  ...
  cacheDir: process.env.TC_VITE_CACHE_DIR ?? path.join(ROOT, '.vite', 'probe-tiber'),
  timeoutMs: 150000,
});
