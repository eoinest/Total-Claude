...
export async function boot({ port, map, scenario = 'assault', tier = 'ultra', size, out, label,
  ...
  /*
   * 22 Aug 2026 — through the budget, not around it. This rig used `chromium.launch` directly,
   * which is exactly the shape that put this machine at load 160: a 12-seed sweep is the most
   * tempting thing in the repository to parallelise. `launchBrowser` takes a machine-wide slot
   * (cap 4), defaults the same GPU arguments this call spelled out, and releases on `close()`,
   * so the call sites below are unchanged.
   */
  const browser = await launchBrowser({
    label: label ? `judge/${label}` : 'judge', port, root: ROOT,
    args: ['--hide-scrollbars', '--enable-unsafe-webgpu', '--autoplay-policy=no-user-gesture-required'],
  });
