#!/usr/bin/env node
/**
 * Cut a trailer out of the live simulation.
 *
 * -------------------------------------------------------------------------------------
 ...
 *   node tools/scratch/trailer-cut.mjs --port=5219              # the whole thing
 *   node tools/scratch/trailer-cut.mjs --port=5219 --beats=gate,ram
 *   node tools/scratch/trailer-cut.mjs --encode                 # frames -> webm, no capture
 */
...
const OUT = args.get('out') ?? 'docs/video/total-claude-trailer.webm';
...
const BEATS = [
  ...
  {
    id: 'rome-ram', scene: 'rome-assault', at: [202, 208],
    desc: 'The ram under its shed at the Porta Flaminia, three-quarter on, blows landing.',
    anchor: { kind: 'bay', k: 0, subject: 'gate' },
    from: { stand: 10, lift: 0, eye: 11, aim: 3.8, dist: 46, fov: 34, yaw: 'in', yawAdd: -0.98 },
    to:   { stand: 8, lift: 0, eye: 8, aim: 3.4, dist: 33, fov: 34, yaw: 'in', yawAdd: -0.74 },
    fadeIn: 9,
  },
  {
    id: 'rome-gate', scene: 'rome-assault', at: [210, 218],
    desc: 'The twenty-sixth blow: the leaves of the Porta Flaminia give way at t+215.',
    anchor: { kind: 'bay', k: 0, subject: 'gate' },
    from: { stand: 5, lift: 0, eye: 8.5, aim: 3.8, dist: 44, fov: 32, yaw: 'in', yawAdd: -0.62 },
